...
class AgentTeamService:
    ...
    async def send_message(self, sender_id: str, to_agent_id: str, content: str) -> Relay:
        """发送消息给指定 Agent。"""
        ...
        await self._msg_repo.save(message)
        return message
    ...
    async def receive_message(
        ...
        if messages:
            ...
            await self._msg_repo.mark_as_read([item.relay.id for item in messages])
    ...
    async def get_contacts(
        ...
        agents = [agent for agent in agents if agent.status != "deleted"]
        if status_filter is not None:
            agents = [agent for agent in agents if agent.status == status_filter]
        return agents

    async def get_contact_detail(self, agent_id: str) -> Agent:
        """查询 Agent 详情。"""
